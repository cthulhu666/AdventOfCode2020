-- This could be fairly easy generalized for N dimensions,
-- but I will just adjust 3D code for 4D.
...
parse str = World bounds points
    where
        ...
        xs = map mapRow rows
        points = [(x,y,0,0) | (y, r) <- enumerate xs, x <- r]
        bounds = ((0, w-1), (0, h-1), (0, 0), (0, 0))
        w = length $ head rows
        h = length rows
...
inflateBounds ((x1,x2), (y1,y2), (z1,z2), (w1,w2)) = ((x1-1, x2+1), (y1-1, y2+1), (z1-1, z2+1), (w1-1, w2+1))


iterateInactive :: World -> [(Point, Int)]
iterateInactive (World ((x1,x2), (y1,y2), (z1,z2), (w1,w2)) points) = map (activeNeighbours points) ys
    where
        xs = [(x,y,z,w) | x <- [x1..x2], y <- [y1..y2], z <- [z1..z2], w <- [w1..w2]]
        ys = filter (\p -> not (p `elem` points)) xs


activeNeighbours :: [Point] -> Point -> (Point, Int)
activeNeighbours xs (x,y,z,w) = ((x,y,z,w), length ns)
    where ns = filter (\(x',y',z',w') -> (abs (x'-x)) <= 1 && (abs (y'-y)) <= 1 && (abs (z'-z)) <= 1 && (abs (w'-w)) <= 1) xs
